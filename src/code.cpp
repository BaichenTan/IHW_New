...
//creates the difference matrix 
//does things for the lambda max
...
//prox = argmin 0.5||t-x||^2 + \lam Regularizer(t)
...
// just a search method
// used for FDR type of things
...
class Grenander {
public:
  //x_knots are the t threshold (locs)
  //y_knots are the F (distribution function)
  //slope_knots are the f (density function)
  //this is 0 based but R is 1 based!!! (very important)
  NumericVector x_knots;
  NumericVector y_knots;
  NumericVector slope_knots;

  //ease for computation: doesn't really matter for now
  int length;
  ...
  // Some helper functions in the Grenander class
  ...
  //input lambda
  //finds the interval in which the pdf is equal to lambda
  std::pair<double, double> invert_subgradient(double lambda) {
    ...
    //returns the two tupple of the lower and upper bound
    return std::make_pair(loc_lb, loc_ub);
  }

  //this function is needed for the ADMM algorithm: look at the paper to figrue out it
...
//These functions are used to check the grenander class is implemented correctly
//' @export
// [[Rcpp::export]]
...
double bisection(double a, double b, double tol, Func&& fun, bool use_geometric_mean = false) {
  ...
  //should be b - a instead of fb - fa? !!!
  while (b - a > tol) {
...
//Defines a GrenMix class
//The type mixture of grenander objects
//list of the grenanders objects
class GrenMix {
public:
  std::vector<Grenander*> grenanders; // Store pointers to Grenander objects
  //different m s
  //could add p0 later
  NumericVector test_ms; // this is mg
  std::vector<double> ts_values;
  std::vector<double> Fs_values;

  //creates the C++ grenander objects
  ...
  //check whether FDR is on the left or the right of alpha
  std::pair<double, double> lagrange_balance(double lambda, double alpha, bool linearized) {
    ...
    double Fdr_L, Fdr_R;
    //this method used linearized to do the interpolation
    if (linearized) {
  ...
  //similar to the subgradient_tilted
  ...
  //implement the unpenalized case of the bh: soluton for lambda = 0
  double unregularized_thresholds_bh_lambda(double alpha) {
  ...
  NumericMatrix total_variation_path_ts_new(double alpha, const NumericVector& lambda_multipliers); //defined below
...
//this code does the lambda = infinity case
std::pair<double, double> GrenMix::single_t_FDR(double alpha) {
  ...
  //translation betwen lagrangian and pdf
  mu_dual = pdf(t) / (1 - alpha * pdf(t));
  //t is the universal rejection threshold
...
//Defines a total_variation_path_ts function for the GrenMix class
NumericMatrix GrenMix::total_variation_path_ts(double alpha, const NumericVector& lambda_multipliers) {
  
  //a type in Rcpp representing a numeric matrix.
  int num_grenanders = this->grenanders.size();
  //This line creates a numeric matrix named ts_array with a number of rows equal 
  //to num_grenanders and a number of columns equal to lambda_multipliers.size() + 2.
  ...
  //get lambdas by lambda_max * lambda_multipliers
  ...
  double tol = alpha * std::pow(10.0, -3.0);  // Set tolerance.
  ...
  for(int i = 0; i < lambdas.size(); ++i) {
    ...
    for (int j = 0; j < 250; ++j) { // should replace this with an actual stopping rule
      ...
      if (relative_change < 1e-3 && l2_sum < 1e-3 && dualres_sum < 1e-3) {
...
// [[Rcpp::export]]
double unregularized_thresholds_bh_lambda(List train_grenander, NumericVector test_ms, double alpha) {
  GrenMix grenmix(train_grenander, test_ms);
  return grenmix.unregularized_thresholds_bh_lambda(alpha);
}
...
//can use this, but needs the mu_dual
//need mu_dual
//
// [[Rcpp::export]]
double lambdaMax(List train_grenander, NumericVector test_ms, double alpha, double mu_dual, double single_t) {
 ...
 NumericMatrix optimal_ts(List train_grenander, NumericVector test_ms, double alpha, NumericVector lambda_multipliers) {
   //This line creates an instance of the GrenMix class by calling its constructor.
   //The GrenMix class is initialized with the train_grenander list and the test_ms numeric vector.
   GrenMix gren_mix(train_grenander, test_ms);
   //line calls the member function total_variation_path_ts of the gren_mix object.
