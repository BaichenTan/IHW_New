...
library("IHW")
ihwRes_old <- IHWold::ihw(pvalue ~ baseMean,  data = deRes, alpha = 0.1)
ihwRes_admm <- ihw(pvalue ~ baseMean,  data = deRes, alpha = 0.1, admm = T)
ihwRes_cvxr <- ihw(pvalue ~ baseMean,  data = deRes, alpha = 0.1, admm = F)
```

```{r runtime}
system.time(ihwRes_admm <- IHW::ihw(pvalue ~ baseMean,  data = deRes, alpha = 0.1, admm = T))[1]
system.time(ihwRes_old <- IHWold::ihw(pvalue ~ baseMean,  data = deRes, alpha = 0.1))[1]
```
...
x <- c(1:47)
for (i in 1:5){
  y1 <- (ihwRes_admm@weights[, i])
  y2 <- (ihwRes_cvxr@weights[, i])
  y3 <- (ihwRes_old@weights[, i])
  # Plot the first plot
  plot(x, y1, type = "l", col = "blue", ylim = c(0, max(y1, y2)), xlab = "groups", ylab = "weights")
  
  # Add the second and third plot
  lines(x, y2, type = "l", col = "red")
  
  lines(x, y3, type = "l", col = "green")
  
  legend("topright", legend = c("New IHW", "CVXR", "Old IHW"),
...
```{r rejections}
rejections(ihwRes_admm)
rejections(ihwRes_old)
rejections(ihwRes_cvxr)
```

And we can also extract the adjusted p-values:
```{r headadjp}
head(adj_pvalues(ihwRes_admm))
head(adj_pvalues(ihwRes_old))
head(adj_pvalues(ihwRes_cvxr))
sum(adj_pvalues(ihwRes_admm) <= 0.1, na.rm = TRUE) == rejections(ihwRes_admm)
sum(adj_pvalues(ihwRes_old) <= 0.1, na.rm = TRUE) == rejections(ihwRes_old)
sum(adj_pvalues(ihwRes_cvxr) <= 0.1, na.rm = TRUE) == rejections(ihwRes_cvxr)
```
...
stopifnot( sum(padjBH <= 0.1, na.rm = TRUE) < 0.9 * rejections(ihwRes_admm) )
stopifnot( sum(padjBH <= 0.1, na.rm = TRUE) < 0.9 * rejections(ihwRes_old) )
stopifnot( sum(padjBH <= 0.1, na.rm = TRUE) < 0.9 * rejections(ihwRes_cvxr) )
```
...
```{r headweights}
head(weights(ihwRes_admm))
head(weights(ihwRes_old))
head(weights(ihwRes_cvxr))
```
...
```{r}
c(nbins(ihwRes_admm), nfolds(ihwRes_admm))
c(nbins(ihwRes_old), nfolds(ihwRes_old))
c(nbins(ihwRes_cvxr), nfolds(ihwRes_cvxr))
```
...
```{r}
weights(ihwRes_admm, levels_only = TRUE)
weights(ihwRes_old, levels_only = TRUE)
weights(ihwRes_cvxr, levels_only = TRUE)
```
...
#plot(ihwRes_admm, what = "decisionboundary") 
#plot(ihwRes_old, what = "decisionboundary") 
#plot(ihwRes_cvxr, what = "decisionboundary") 
```
...
library("ggplot2")
gg <- ggplot(as.data.frame(ihwRes_admm), aes(x = pvalue, y = adj_pvalue, col = group)) + 
  geom_point(size = 0.25) + scale_colour_hue(l = 70, c = 150, drop = FALSE)
gg
gg %+% subset(as.data.frame(ihwRes_admm), adj_pvalue <= 0.2)
```

```{r}
gg <- ggplot(as.data.frame(ihwRes_old), aes(x = pvalue, y = adj_pvalue, col = group)) + 
  geom_point(size = 0.25) + scale_colour_hue(l = 70, c = 150, drop = FALSE)
gg
gg %+% subset(as.data.frame(ihwRes_old), adj_pvalue <= 0.2)
```
```{r}
gg <- ggplot(as.data.frame(ihwRes_cvxr), aes(x = pvalue, y = adj_pvalue, col = group)) + 
  geom_point(size = 0.25) + scale_colour_hue(l = 70, c = 150, drop = FALSE)
gg
gg %+% subset(as.data.frame(ihwRes_cvxr), adj_pvalue <= 0.2)
```

The `r class(ihwRes)` object `ihwRes` can be converted to a dataframe that contains the following columns:

```{r}
ihwResDf <- as.data.frame(ihwRes_admm)
colnames(ihwResDf)
```
...
```{r Bonferroni}
ihwBonferroni_admm <- ihw(pvalue ~ baseMean, data = deRes, alpha = 0.1, adjustment_type = "bonferroni", admm = T)
ihwBonferroni_old <- IHWold::ihw(pvalue ~ baseMean, data = deRes, alpha = 0.1, adjustment_type = "bonferroni")
#ihwBonferroni_cvxr <- IHWold::ihw(pvalue ~ baseMean, data = deRes, alpha = 0.1, adjustment_type = "bonferroni", admm = F)
...
```{r ihw2}
ihwS_old <- IHWold:::ihw(p ~ group, alpha = 0.1, data = filter(sim, reported), m_groups = m_groups)
ihwS_admm <- IHW:::ihw(p ~ group, alpha = 0.1, data = filter(sim, reported), m_groups = m_groups, admm = T)
ihwS_old
ihwS_admm
```
...
```{r ihw12}
ihwF_old <- IHWold:::ihw(p ~ group, alpha = 0.1, data = sim)
ihwF_admm <- IHW:::ihw(p ~ group, alpha = 0.1, data = sim, admm = T)
```
...
#  plot(ihwF_old), 
#  ncol = 2) 
c(rejections(ihwS_old),
  rejections(ihwF_old))
c(rejections(ihwS_admm),
  rejections(ihwF_admm))
```
...
qplot(adj_pvalues(ihwF_old)[sim$reported], adj_pvalues(ihwS_old), cex = I(0.2), 
      xlim = c(0, 0.1), ylim = c(0, 0.1)) + coord_fixed()
qplot(adj_pvalues(ihwF_admm)[sim$reported], adj_pvalues(ihwS_admm), cex = I(0.2), 
      xlim = c(0, 0.1), ylim = c(0, 0.1)) + coord_fixed()
```
